import { validationRunSchema } from "@hivemind/schema";
...
export async function POST(
  ...
  return withAuth(request, { learner: true }, async (principal) => {
    const body = await readJson(request, (value) => validationRunSchema.parse(value));
    if (!body.ok) {
      return body.response;
    }
    const { id } = await context.params;
    const { workOrders } = await services();
    const result = await workOrders.recordValidation(
      id,
      body.value,
